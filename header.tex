\documentclass[titlepage,twoside,BCOR=12mm,parskip=half,captions=tableheading,a4paper]{scrartcl}

\input{packages.tex}

\ExplSyntaxOn

\hypersetup{pdfauthor={Igor~Babuschkin}}

\input{header/modules/base.tex}
\input{header/modules/comment.tex}
\input{header/modules/floats.tex}
\input{header/modules/math.tex}
\input{header/modules/units.tex}
...
% http://stackoverflow.com/a/1569922
\makeatletter
...
\renewenvironment{table}[1][\fps@table]{
  ...
  \@tempa\centering\sisetup{per-mode=reciprocal}
}{
  \end@float
  \ignorespacesafterend
}
...
\newcommand \thetitle
{
        Flavour-Tagging-Kalibration~des~SS\Pgp-Taggers~mittels~$\PBzero \to \PJpsi\,\PKst^0$-Zerfällen~am~LHCb-Experiment
}
